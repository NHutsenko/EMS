...
namespace EMS.Structure.Team.Application.Services;
...
public sealed class TeamService : Protos.TeamService.TeamServiceBase
{
    ...
    public override async Task GetAll(Empty request, IServerStreamWriter<Protos.Team> responseStream, ServerCallContext context)
    {
        IEnumerable<Protos.Team> data = (await _teamRepository.GetAllAsync(context.CancellationToken))
            .Select(e => new Protos.Team
            {
                Id = e.Id,
                Name = e.Name,
                Members =
